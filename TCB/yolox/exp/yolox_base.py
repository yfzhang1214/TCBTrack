...
class Exp(BaseExp):
    ...
    def get_model(self, settings=None):
        ...
        return self.model
    def get_model2(self, settings=None):
        from yolox.models import YOLOPAFPN, YOLOX2, YOLOXHead2
        def init_yolo(M):
            for m in M.modules():
                if isinstance(m, nn.BatchNorm2d):
                    m.eps = 1e-3
        ...
        if getattr(self, "model", None) is None:
            ...
            if settings is not None:
                ...
                nID = 2		# default value should not be 1
            backbone = YOLOPAFPN(self.depth, self.width, in_channels=in_channels)
            head = YOLOXHead2(self.num_classes, self.width, in_channels=in_channels, nID=nID)
    ...
    def get_optimizer(self, batch_size):
        if "optimizer" not in self.__dict__:
            ...
            for k, v in self.model.named_modules():
                if "reid" not in k:#############################################################only train reid head!!!
                    continue
            ...
            #)
            optimizer = torch.optim.SGD(
                pg2, lr=lr, momentum=self.momentum, nesterov=True
            )
