...
def _mirror(image, boxes,pd=None):
...
def preproc(image, input_size, mean, std, swap=(2, 0, 1)):
    ...
    img = np.array(image)
    r = min(input_size[0] / img.shape[0], input_size[1] / img.shape[1])#放大倍数
    ...
    padded_img /= 255.0#normalization
    if mean is not None:
        padded_img -= mean
    if std is not None:
        padded_img /= std
    padded_img = padded_img.transpose(swap)#C,H,W
...
class TrainTransform:
    ...
    def __call__(self, image, targets, input_dim,pd=None):
        ...
        image_t = _distort(image,pd)
        #print(boxes[0])
        image_t, boxes = _mirror(image_t, boxes,pd)
        #print(boxes[0])
