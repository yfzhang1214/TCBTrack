...
#from yolox.tracker.my_byte_tracker_mot17 import BYTETracker
...
class MOTEvaluator:
    ...
    def evaluate_TCB(
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            #im = cv2.imread(os.path.join("/home/yfzhang/DanceTrack/ByteTrack_ReID/datasets/mot/train/",info_imgs[4][0]))
            #vis_path = os.path.join("/home/yfzhang/DanceTrack/ByteTrack_ReID/vis/",(info_imgs[4][0]).split("/")[0])
            ...
            with torch.no_grad():
                ...
                if video_name == 'MOT17-01-FRCNN':
                    ...
                    self.args.track_thresh = ori_thresh
                params=[0.7,0.4,0.2,-100,0.1]

                if video_name not in video_names:
                    video_names[video_id] = video_name
                if frame_id ==1:
                    #print(tracker.num1,tracker.num2,tracker.num3,tracker.num4)
                    tracker = BYTETracker(self.args,params)#new tracker
                    if video_name == 'MOT17-13-FRCNN':
                        from yolox.tracker.my_byte_tracker_mot17_kal import BYTETracker as bytetracker2
                        tracker = bytetracker2(self.args,params)
            ...
            if outputs[0] is not None:
                ...
                #cv2.imwrite(os.path.join("/home/yfzhang/DanceTrack/ByteTrack_ReID/vis/",info_imgs[4][0]),im)
                # save results
                results.append((frame_id, online_tlwhs, online_ids, online_scores))
            if is_time_record:
                track_end = time_synchronized()
