...
class STrack(BaseTrack):
    ...
    def update_features(self, feat):
        ...
        if(self.curr_feat == None):
            ...
            self.curr_feat = self.curr_feat*0.1+feat*0.9
    ...
    def update(self, output_results, img_info, img_size, id_feature=None):
        ...
        for itracked, idet in matches:          # do update w.r.t matching results
            ...
            if track.state == TrackState.Tracked:       # normally tracked successfully tracked
                track.update(detections[idet], self.frame_id,detections[idet].score>0.9)   # update of class STrack, update Kalman Filter, track state and other settings
        ...
        for itracked, idet in matches:  # do update w.r.t matching results
            ...
            if track.state == TrackState.Tracked:
                track.update(detections[idet],
                             self.frame_id,detections[idet].score>0.9)  # update of class STrack, update Kalman Filter, track state and other settings
        ...
        for itracked, idet in matches:
            unconfirmed[itracked].update(detections[idet], self.frame_id,detections[idet].score>0.9)
