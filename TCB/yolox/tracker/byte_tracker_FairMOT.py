...
class BYTETracker(object):
    ...
    def update(self, output_results, img_info, img_size, id_feature):  # TODO: ReID. add 'id_feature'
        ...
        remain_inds = scores > self.args.track_thresh#0.5
        inds_low = scores > 0.1
        ...
        for track in self.tracked_stracks:
            if not track.is_activated:
                ...
                tracked_stracks.append(track)#tracked_stracks:用来保存上一帧的所有tracker信息

        ''' Step 1: First association, with embedding'''  # TODO: borrowed from FairMOT
        strack_pool = joint_stracks(tracked_stracks, self.lost_stracks) #上一帧出现的所有框和之前loss掉但没delete的框（30frames）
        # Predict the current location with KF
        STrack.multi_predict(strack_pool) #根据KF算出先前框的预测框
        dists = matching.embedding_distance(strack_pool, detections)#当前检测到的high score框和先前的跟踪框和暂时lost框进行distance计算
        ...
        r_tracked_stracks = [strack_pool[i] for i in u_track if strack_pool[i].state == TrackState.Tracked]#未被匹配的先前关注的框
...
def remove_duplicate_stracks(stracksa, stracksb):#如果两段track离得近，需要去掉一个，取决于出现的帧数多少
    pdist = matching.iou_distance(stracksa, stracksb)
    pairs = np.where(pdist < 0.15)#IOU>0.85,把时间跨度小的tracker去掉
    dupa, dupb = list(), list()
