...
class BYTETracker(object):
    def __init__(self, args, params=[-100,0.9,100,-100],frame_rate=30):
        ...
        self.det_thresh = 0.8#args.track_thresh + 0.1
    ...
    def update(self, output_results, img_info, img_size, id_feature=None):
        ...
        if(len(strack_pool)!=0):
            matches, u_track = match1(strack_pool,id_feature,scores,bboxes)
            
            #print(len(matches)/len(strack_pool))
            #kkk1 = []
            #kkk2 = []
            #for i,j in matches:
            #    kkk1.append(strack_pool[i].tlbr)
            #    kkk2.append(bboxes[j])
            #kkk1 = np.asarray(kkk1)
            #kkk2 = np.asarray(kkk2)
            if(len(matches)>0):
                for itracked, idet in matches:
                    track = strack_pool[itracked]
                    det = STrack(STrack.tlbr_to_tlwh(bboxes[idet]), scores[idet], id_feature[idet], 60)
                    if track.state == TrackState.Tracked:
                        track.update(det, self.frame_id,scores[idet]>0.8)
                ...
                bboxes = bboxes[mask==True,:]
                id_feature = id_feature[mask==True,:]
        ...
        if(len(strack_pool)>0 and len(detections)>0):
            motion = match2(strack_pool,id_feature_keep)
            s2 = match3(strack_pool,id_feature_keep)
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, f, 60) for  # detections_second  --> STrack
        ...
        if(len(r_tracked_stracks)>0 and len(detections_second)>0):
            motion = match2(r_tracked_stracks,id_feature_second,pd=-100)
...
def match2(strack_pool,id_feature,pd=-100):
    ...
    output = M<pd#########################################################################################
    return output
def match1(starck_pool,id_feature,scores,bboxes):
    track_list = []
    tl = []
    ...
    for i in range(0,len(starck_pool)):
        ...
        if(scores[index[i]]>0.3 and ratio>0.5 and ratio<2 and iou>0.5):
            if(__[i]>0.9):#find pair
