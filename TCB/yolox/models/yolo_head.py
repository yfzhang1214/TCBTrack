...
class YOLOXHead(nn.Module):
    ...
    def forward(self, xin, labels=None, imgs=None):     # xin: 256/8, 512/16, 1024/32
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                ...
                )#[1,4+1+1+128,100,180],[1,4+1+1+128,50,90],[1,4+1+1+128,25,45]
    ...
    def decode_outputs(self, outputs, dtype):
        ...
        for (hsize, wsize), stride in zip(self.hw, self.strides):
            ...
            grids.append(grid)
            shape = grid.shape[:2]#grid
            strides.append(torch.full((*shape, 1), stride))

        grids = torch.cat(grids, dim=1).type(dtype)
        strides = torch.cat(strides, dim=1).type(dtype)

        outputs[..., :2] = (outputs[..., :2] + grids) * strides#x,y
        outputs[..., 2:4] = torch.exp(outputs[..., 2:4]) * strides#h,w
        return outputs
    ...
    def get_losses(
        ...
        if mixup:
            label_cut = labels[..., :5]#把track id去掉了
        else:
            label_cut = labels
        nlabel = (label_cut.sum(dim=2) > 0).sum(dim=1)  # number of objects

        total_num_anchors = outputs.shape[1]#23625
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                torch.cuda.empty_cache()
                num_fg += num_fg_img#num_fg_img:最后筛选出的框的个数，用于计算loss
