...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        ...
        for seq in sorted(seqs):
            ...
            count=0
            for i in range(0,num_images-1,fr):####
                count+=1
                img = cv2.imread(os.path.join(data_path, '{}/img1/{:06d}.jpg'.format(seq,i+1)))
                height, width = img.shape[:2]
                image_info = {'file_name': '{}/img1/{:06d}.jpg'.format(seq,i+1),
                              'id': image_cnt + count,#整个training set的image id，从1算起
                              'frame_id':count,
                              'prev_image_id': image_cnt + count-1 if i>0 else -1,
...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        ...
        for seq in sorted(seqs):
            ...
            count = 0
            for i in range(num_images//2, num_images,fr):
                count+=1
                img = cv2.imread(os.path.join(data_path, '{}/img1/{:06d}.jpg'.format(seq,i+1)))
                height, width = img.shape[:2]
                image_info = {'file_name': '{}/img1/{:06d}.jpg'.format(seq,i+1),
            ...
            anns = np.loadtxt(ann_path, dtype = np.float32, delimiter=',')
            anns_out = np.array([anns[i] for i in range(anns.shape[0]) if int(anns[i][0])-1>=num_images//2 and (int(anns[i][0])-num_images//2)%fr==1],np.float32)
            anns_out[:,0] = (anns_out[:,0]-num_images//2+fr-1)//fr
            gt_out = os.path.join(seq_path, 'gt/gt_{}.txt'.format(split))
            fout = open(gt_out,'w')
            for o in anns_out:
                fout.write('{:d},{:d},{:d},{:d},{:d},{:d},{:d},{:d},{:.6f}\n'.format(
            ...
            image_cnt+=(num_images-num_images//2+fr-1)//fr
            print(tid_curr,tid_last)
        print('loaded {} for {} images and {} samples'.format(split, len(out['images']), len(out['annotations'])))
        json.dump(out, open(out_path, 'w'))