...
class Predictor(object):
    ...
    def inference(self, img, timer):
        ...
        img = torch.from_numpy(img).unsqueeze(0).float().to(self.device)#add batch dim
        if self.fp16:
        ...
        return outputs, img_info#[num_bbox, 135(4+1+1+1+128,bbox+obj+class_conf+classid+embedReID)]
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    timestamp = time.strftime("%Y_%m_%d_%H_%M_%S", current_time)
    save_folder = osp.join(vis_folder, timestamp)
    os.makedirs(save_folder, exist_ok=True)################################################################################到这里
    if args.demo == "video":
    ...
    while True:
        ...
        if ret_val:
            ...
            if outputs[0] is not None:# detect object
